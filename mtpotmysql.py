...
db_host = HOSTANAMEorIP
db_username = USERNAME
db_password = USERPASWORD
db_dbname = NAMEOFDATABASE
...
class MyTelnetHandler(TelnetHandler, object):
    ...
    def store_command(self, cmd):
        honey_logger.debug(
            ...
            self.client_address[1],
            cmd.strip())
		db = MySQLdb.connect(db_host,db_username,db_password,db_dbname)
		cursor = db.cursor()
        cursor.execute("INSERT INTO cmds(ip, cmd) VALUES (%s, %s)", (self.client_address[0], cmd))
        db.commit()
		db.close()
        if syslogger:
            syslogger.debug(
    ...
    def authCallback(self, username, password):
		...
		db = MySQLdb.connect(db_host,db_username,db_password,db_dbname)
        cursor = db.cursor()
        cursor.execute("INSERT INTO logins(ip, user, pass) VALUES (%s, %s, %s)", (self.client_address[0], username, password))
        db.commit()
		db.close()
